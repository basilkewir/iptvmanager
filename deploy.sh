...
if [ "$DISK_PCT" -ge 90 ]; then
    echo "  ERROR: Disk is ${DISK_PCT}% full -- aborting to protect DVR data."
...
if [ "$LOCAL" = "$REMOTE" ]; then
    ...
    echo "  OK: Code updated ($(git log -1 --format='%h %s'))"
fi
...
if [ ! -f "$DB_PATH" ]; then
    echo "  INFO: No database found -- it will be created on first start."
else
    python3 - <<'PYEOF'
import sqlite3, sys
...
if not cur.fetchone():
    print("  INFO: streams table not yet created -- will be made on first start.")
    conn.close()
    sys.exit(0)
...
if added:
    ...
    print("  OK: Schema is up to date -- no changes needed.")
PYEOF
fi
